...
export class ValidateTransactionHistoryReturnSellerTransactionRentalUseCase {
  constructor(
  ...
  async execute(
    ...
    if (transactionRental.status === "WaitingForReturnConfirmation") {
      ...
      const matchingItems = filterTransactionParticipantsId.some(
        (offer: Daum) => {
          return transactionRental.skinsRent.every((skin) => {
            return offer.items.received.some((item) => {
              return (
      ...
      console.log(
        "filterTransactionParticipantsItems -------- SELLER",
        matchingItems
      );

      if (matchingItems) {
        console.log("Entrou aqui 2 ---- SELLER");
